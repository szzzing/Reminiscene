...
axios.interceptors.response.use(
    (config) => {
        ...
        if(store.state.auth.accessToken!=null && config.headers.access_token==null) {
            store.commit("auth/logout");
            store.commit("modal/setAlert", { alertEmoji:"⚠️", alertText:"인증이 만료되어 로그아웃되었어요." });
        }
        // 토큰 저장
        store.commit("auth/setAccessToken", config.headers.access_token);
        // 사용자 정보 저장
    ...
    (error) => {
        ...
        if(error.response.status==403) {
            ...
            // 2) 로그인이 되어있으면 에러페이지로
            } else if(store.state.auth.accessToken!=null) {
